import { Pool } from "pg";
import { CreateFinancialGoalInput } from "../types/internal";
import { UserMemoryRow } from "../../user/types/entity/user-memory.entity";
import { MonthlyFinanceRow } from "../../financial/types/entity/monthly-finances.entity";
import { FinancialGoalRow } from "../../financial/types/entity/financial-goal.entity";
import { FinancialStatementRow } from "../../financial/types/entity/financial-statement.entity";

/**
 * AI Tool 에서 사용하는 공용 Repository
 */
export class ChatToolRepository {
  constructor(private readonly db: Pool) {}

  // ─── User Memory ─────────────────────────────────────────

  async findUserMemoryByUserId(userId: string): Promise<UserMemoryRow | null> {
    const result = await this.db.query(
      `SELECT * 
       FROM user_memories 
       WHERE user_id = $1 
       ORDER BY updated_at DESC 
       LIMIT 1`,
      [userId],
    );

    return (result.rows[0] as UserMemoryRow) ?? null;
  }

  async upsertUserMemory(
    userId: string,
    content: { 메모리: string; 중요정보: string },
  ): Promise<void> {
    const existing = await this.db.query(
      `SELECT id FROM user_memories WHERE user_id = $1 LIMIT 1`,
      [userId],
    );

    if (existing.rows.length > 0) {
      await this.db.query(
        `UPDATE user_memories 
         SET content = $1, updated_at = CURRENT_TIMESTAMP 
         WHERE user_id = $2`,
        [content, userId], // JSONB 컬럼이면 stringify 필요 없음
      );
    } else {
      const { v4: uuidv4 } = await import("uuid");

      await this.db.query(
        `INSERT INTO user_memories (id, user_id, content) 
         VALUES ($1, $2, $3)`,
        [uuidv4(), userId, content],
      );
    }
  }

  // ─── Monthly Finances ─────────────────────────────────────

  async findMonthlyFinance(
    userId: string,
    year: number,
    month: number,
  ): Promise<MonthlyFinanceRow | null> {
    const result = await this.db.query(
      `SELECT * 
       FROM monthly_finances 
       WHERE user_id = $1 AND year = $2 AND month = $3`,
      [userId, year, month],
    );

    return (result.rows[0] as MonthlyFinanceRow) ?? null;
  }

  async findMonthlyFinanceRange(
    userId: string,
    startYear: number,
    startMonth: number,
    endYear: number,
    endMonth: number,
  ): Promise<MonthlyFinanceRow[]> {
    const result = await this.db.query(
      `SELECT * 
       FROM monthly_finances
       WHERE user_id = $1
         AND (year * 100 + month) BETWEEN $2 AND $3
       ORDER BY year ASC, month ASC`,
      [userId, startYear * 100 + startMonth, endYear * 100 + endMonth],
    );

    return result.rows as MonthlyFinanceRow[];
  }

  // ─── Financial Goals ──────────────────────────────────────

  async findGoalsByUserId(userId: string): Promise<FinancialGoalRow[]> {
    const result = await this.db.query(
      `SELECT * 
       FROM financial_goals 
       WHERE user_id = $1 
       ORDER BY created_at ASC`,
      [userId],
    );

    return result.rows as FinancialGoalRow[];
  }

  async createGoal(input: CreateFinancialGoalInput): Promise<void> {
    await this.db.query(
      `INSERT INTO financial_goals
        (id, user_id, name, description, target_amount, current_amount, monthly_contribution, start_date, end_date)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [
        input.userId,
        input.name,
        input.description ?? null,
        input.targetAmount,
        input.currentAmount ?? 0,
        input.monthlyContribution ?? 0,
        input.startDate ?? null,
        input.endDate ?? null,
      ],
    );
  }

  // ─── Financial Statements ─────────────────────────────────

  async findLatestStatementByUserId(
    userId: string,
  ): Promise<FinancialStatementRow | null> {
    const result = await this.db.query(
      `SELECT * 
       FROM financial_statements 
       WHERE user_id = $1 
       ORDER BY created_at DESC 
       LIMIT 1`,
      [userId],
    );

    return (result.rows[0] as FinancialStatementRow) ?? null;
  }

  async findStatementsByUserId(
    userId: string,
  ): Promise<FinancialStatementRow[]> {
    const result = await this.db.query(
      `SELECT * 
       FROM financial_statements 
       WHERE user_id = $1 
       ORDER BY created_at DESC`,
      [userId],
    );

    return result.rows as FinancialStatementRow[];
  }

  async createStatement(input: {
    id: string;
    userId: string;
    netMonthlyIncome: number;
    monthlyFixedExpenses: Record<string, number> | null;
    monthlySavingsInvestment: Record<string, number> | null;
    info: string;
  }): Promise<void> {
    await this.db.query(
      `INSERT INTO financial_statements
        (id, user_id, net_monthly_income, monthly_fixed_expenses, monthly_savings_investment, info, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, CURRENT_TIMESTAMP)`,
      [
        input.id,
        input.userId,
        input.netMonthlyIncome,
        input.monthlyFixedExpenses, // JSONB이면 그대로
        input.monthlySavingsInvestment,
        input.info,
      ],
    );
  }

  async deleteOldestStatementIfExceedsLimit(userId: string): Promise<void> {
    const statements = await this.findStatementsByUserId(userId);

    if (statements.length >= 2) {
      const oldest = statements[statements.length - 1];

      await this.db.query(`DELETE FROM financial_statements WHERE id = $1`, [
        oldest.id,
      ]);
    }
  }
}
